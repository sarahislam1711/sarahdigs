  ...
  app.post("/api/admin/seed", isAuthenticated, async (req, res) => {
    try {
      ...
      if (existingHomeContent.length === 0) {
        await storage.upsertPageContent({ pageSlug: "home", sectionKey: "hero", content: { rotatingWords: ["goals", "users", "data", "content"], description: "I help brands find the hidden gold in their analytics, content, and user journeys. No fluff, just deep excavation for growth.", ctaText: "Start Digging" } });
        ...
        // Force-update hero rotating words to remove outdated entries
        const heroContent = existingHomeContent.find(c => c.sectionKey === "hero");
        if (heroContent) {
          const content = heroContent.content as Record<string, unknown>;
          await storage.upsertPageContent({ pageSlug: "home", sectionKey: "hero", content: { ...content, rotatingWords: ["goals", "users", "data", "content"] } });
        }
      }
